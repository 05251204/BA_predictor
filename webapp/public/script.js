...
charForm.addEventListener('submit', async (event) => {
  ...
  try {
        const response = await fetch('/api/predict-characters', {
            method: 'POST',
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('ページがロードされました。バックエンドからCSVデータを取得します...');
    ...
    try {
        ...
        console.log('バックエンドからCSVデータを受け取りました:', data);

        // 受け取ったデータをHTMLに表示する例
        if (data && data.length > 0) {
            let html = '<ul>';
            data.forEach(row => {
                html += `<li>名前: ${row.Name}, 年齢: ${row.Age}, 都市: ${row.City}</li>`;
            });
            html += '</ul>';
            csvOutputDiv.innerHTML = html;
        } else {
            csvOutputDiv.textContent = 'データがありませんでした。';
        }

    } catch (error) {
        console.error('データの取得に失敗しました:', error);
        csvOutputDiv.textContent = `データの取得中にエラーが発生しました: ${error.message}`;
    }
});
